...
<template>
  <div class="flex h-screen overflow-hidden bg-gray-50">
      ...
      <nav class="flex-1 overflow-y-auto px-2 py-3">
        <ul class="space-y-0.5">
          <li v-for="item in mainSidebarItems" :key="item.id">
            <button
              class="flex w-full cursor-pointer items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-[13px] font-medium transition-colors"
            ...
            >
              <!-- Home solid -->
              <svg v-if="item.icon === 'home'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M11.47 3.841a.75.75 0 0 1 1.06 0l8.69 8.69a.75.75 0 1 0 1.06-1.061l-8.689-8.69a2.25 2.25 0 0 0-3.182 0l-8.69 8.69a.75.75 0 1 0 1.061 1.06l8.69-8.689Z"/>
                <path d="m12 5.432 8.159 8.159c.03.03.06.058.091.086v6.198c0 1.035-.84 1.875-1.875 1.875H15a.75.75 0 0 1-.75-.75v-4.5a.75.75 0 0 0-.75-.75h-3a.75.75 0 0 0-.75.75V21a.75.75 0 0 1-.75.75H5.625a1.875 1.875 0 0 1-1.875-1.875v-6.198a2.29 2.29 0 0 0 .091-.086L12 5.432Z"/>
              </svg>
              <!-- Building solid -->
              <svg v-else-if="item.icon === 'building'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- Credit card solid -->
              <svg v-else-if="item.icon === 'credit-card'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M4.5 3.75a3 3 0 0 0-3 3v.75h21v-.75a3 3 0 0 0-3-3h-15Z"/>
                <path fill-rule="evenodd" d="M22.5 9.75h-21v7.5a3 3 0 0 0 3 3h15a3 3 0 0 0 3-3v-7.5Zm-18 3.75a.75.75 0 0 1 .75-.75h6a.75.75 0 0 1 0 1.5h-6a.75.75 0 0 1-.75-.75Zm.75 2.25a.75.75 0 0 0 0 1.5h3a.75.75 0 0 0 0-1.5h-3Z" clip-rule="evenodd"/>
              </svg>
              <!-- Receipt / Subscriptions solid -->
              <svg v-else-if="item.icon === 'receipt'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path fill-rule="evenodd" d="M5.625 1.5c-1.036 0-1.875.84-1.875 1.875v17.25c0 1.035.84 1.875 1.875 1.875h12.75c1.035 0 1.875-.84 1.875-1.875V12.75A3.75 3.75 0 0 0 16.5 9h-1.875a1.875 1.875 0 0 1-1.875-1.875V5.25A3.75 3.75 0 0 0 9 1.5H5.625ZM7.5 15a.75.75 0 0 1 .75-.75h7.5a.75.75 0 0 1 0 1.5h-7.5A.75.75 0 0 1 7.5 15Zm.75 2.25a.75.75 0 0 0 0 1.5H12a.75.75 0 0 0 0-1.5H8.25Z" clip-rule="evenodd"/>
                <path d="M12.971 1.816A5.23 5.23 0 0 1 14.25 5.25v1.875c0 .207.168.375.375.375H16.5a5.23 5.23 0 0 1 3.434 1.279 9.768 9.768 0 0 0-6.963-6.963Z"/>
              </svg>
              <!-- Globe solid -->
              <svg v-else-if="item.icon === 'globe'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- Users solid -->
              <svg v-else-if="item.icon === 'users'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M4.5 6.375a4.125 4.125 0 1 1 8.25 0 4.125 4.125 0 0 1-8.25 0ZM14.25 8.625a3.375 3.375 0 1 1 6.75 0 3.375 3.375 0 0 1-6.75 0ZM1.5 19.125a7.125 7.125 0 0 1 14.25 0v.003l-.001.119a.75.75 0 0 1-.363.63 13.067 13.067 0 0 1-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 0 1-.364-.63l-.001-.122ZM17.25 19.128l-.001.144a2.25 2.25 0 0 1-.233.96 10.088 10.088 0 0 0 5.06-1.01.75.75 0 0 0 .42-.643 4.875 4.875 0 0 0-6.957-4.611 8.586 8.586 0 0 1 1.71 5.157v.003Z"/>
              </svg>
              <!-- Shield solid -->
              <svg v-else-if="item.icon === 'shield'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path fill-rule="evenodd" d="M12.516 2.17a.75.75 0 0 0-1.032 0 11.209 11.209 0 0 1-7.877 3.08.75.75 0 0 0-.722.515A12.74 12.74 0 0 0 2.25 9.75c0 5.942 4.064 10.933 9.563 12.348a.749.749 0 0 0 .374 0c5.499-1.415 9.563-6.406 9.563-12.348 0-1.39-.223-2.73-.635-3.985a.75.75 0 0 0-.722-.516l-.143.001c-2.996 0-5.717-1.17-7.734-3.08Zm3.094 8.016a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clip-rule="evenodd"/>
              </svg>
              <!-- Fallback solid -->
              <svg v-else class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div class="shrink-0 border-t border-gray-100 px-2 py-2">
        <ul class="space-y-0.5">
          <li v-for="item in footerSidebarItems" :key="item.id">
            ...
            >
              <!-- User solid -->
              <svg v-if="item.icon === 'user'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- Cog solid -->
              <svg v-else-if="item.icon === 'cog'" class="size-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <main
        class="flex-1 overflow-y-auto px-6 py-5 sm:px-10 lg:px-14"
        style="background-color: #f9fafb; background-image: linear-gradient(to right, #e5e7eb 1px, transparent 1px), linear-gradient(to bottom, #e5e7eb 1px, transparent 1px); background-size: 24px 24px;"
      >
        <slot />
      </main>
    </div>
