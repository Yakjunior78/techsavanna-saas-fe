  ...
  >
    ...
    <template v-if="activeSidebarId === 'overview'">
      ...
      <div class="mb-6 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-500 px-5 py-4">
          ...
          <p class="mt-0.5 text-xs text-indigo-100">{{ user?.tenantName || 'My Business' }}</p>
        </div>
        <div class="flex items-center justify-between px-5 py-3">
          ...
          <a
            :href="dashboardUrl"
            class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-indigo-700"
          >
            Open
    ...
    <template v-else-if="activeSidebarId === 'tenants'">
      <div class="mb-5">
        ...
        <p class="mt-0.5 text-xs text-gray-500">Manage your company workspaces.</p>
      </div>
      <div class="space-y-3">
        <a
          :href="dashboardUrl"
    ...
    <template v-else-if="activeSidebarId === 'workspace'">
      <div class="mb-5">
        ...
        <p class="mt-0.5 text-xs text-gray-500">Access your company workspace.</p>
      </div>
      <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-500 px-5 py-4">
          <h2 class="text-sm font-semibold text-white">{{ user?.tenantName || 'My Business' }}</h2>
          <p class="mt-0.5 text-xs text-indigo-100">{{ dashboardUrl }}</p>
        </div>
        <div class="px-5 py-3">
          <a
            :href="dashboardUrl"
