...
const steps: OnboardingStep[] = [
  {
    ...
    description: 'Enter your details to get started with Savanna360ERP',
    icon: 'user',
    required: true,
    order: 1
  },
  {
    ...
    description: 'Help us configure your ERP system',
    icon: 'building',
    required: true,
    order: 2
  },
  {
    ...
    description: 'Choose the billing cycle that fits your enterprise',
    icon: 'credit-card',
    required: true,
    order: 3
  },
...
if (devPreview === 'setup' || devPreview === 'complete') {
  ;['account', 'organization', 'plan', 'verification'].forEach(id => completeStep(id))
}
...
const appFeatures = [
  { icon: 'chart', title: 'Unified Operations', description: 'Connect all departments in one platform' },
  { icon: 'bolt', title: 'Real-time Insights', description: 'Make data-driven decisions instantly' },
  { icon: 'shield', title: 'Enterprise Security', description: 'Role-based access with audit trails' }
]
...
<template>
  ...
  >
      ...
      <div class="space-y-4">
        <div>
          ...
          <div v-if="tenantSubdomain || customSubdomain" class="mt-2">
            ...
            <div class="flex overflow-hidden rounded-lg border border-gray-300 hover:border-gray-400 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100">
              <input
                :value="customSubdomain"
                type="text"
                placeholder="acme-corporation"
                class="w-0 min-w-0 flex-1 bg-white px-3 py-2 text-sm text-violet-600 placeholder:text-xs placeholder:text-gray-400 focus:outline-none"
    ...
    <template v-else-if="currentStep?.id === 'plan'">
      <div class="space-y-4">
        <!-- Trial period info -->
        <div v-if="TRIAL_DAYS" class="flex items-center gap-2.5 rounded-lg border border-violet-200 bg-violet-50/60 px-3.5 py-2.5">
          <svg class="size-4 shrink-0 text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <p class="text-xs text-gray-700">
            <span class="font-semibold text-violet-700">{{ TRIAL_DAYS }}-day free trial</span> — No credit card required. Full access to all features.
          </p>
        </div>

        <div
          class="cursor-pointer rounded-xl border-2 p-3.5 transition-all"
          :class="billingCycle === 'monthly' ? 'border-violet-500 bg-violet-50/50' : 'border-gray-200 hover:border-gray-300'"
          @click="billingCycle = 'monthly'"
        >
          <div class="flex items-center gap-3">
            <div
              class="flex size-5 shrink-0 items-center justify-center rounded-full border-2"
              :class="billingCycle === 'monthly' ? 'border-violet-500 bg-violet-500' : 'border-gray-300'"
            >
        ...
        <div
          class="cursor-pointer rounded-xl border-2 p-3.5 transition-all"
          :class="billingCycle === 'annual' ? 'border-violet-500 bg-violet-50/50' : 'border-gray-200 hover:border-gray-300'"
          @click="billingCycle = 'annual'"
        >
          <div class="flex items-center gap-3">
            <div
              class="flex size-5 shrink-0 items-center justify-center rounded-full border-2"
              :class="billingCycle === 'annual' ? 'border-violet-500 bg-violet-500' : 'border-gray-300'"
            >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-1.5">
                  ...
                  <span class="rounded bg-violet-100 px-1.5 py-0.5 text-[10px] font-medium text-violet-700">Save {{ annualMonthlySaving }}%</span>
                </div>
                <div class="flex items-baseline gap-1">
    ...
    <template v-else-if="currentStep?.id === 'verification'">
      <div class="space-y-5">
        <div class="text-center">
          <div class="mx-auto mb-3 flex size-12 items-center justify-center rounded-full bg-gradient-to-br from-violet-50 to-violet-100">
          ...
          <input
            ...
            :class="[
              otpError
                ...
                : otpDigits[i]
                  ? 'border-violet-300 bg-violet-50/30 text-gray-900 focus:border-violet-500 focus:ring-1 focus:ring-violet-100'
                  : 'border-gray-200 bg-white text-gray-900 focus:border-violet-500 focus:ring-1 focus:ring-violet-100'
            ]"
        ...
        <div class="text-center">
          <p class="text-xs text-gray-400">
            ...
            <button
              ...
              :class="resendCountdown > 0 ? 'cursor-not-allowed text-gray-400' : 'text-violet-600 hover:text-violet-700'"
          ...
          <div class="space-y-6">
            ...
            <div class="rounded-xl border border-gray-100 bg-gray-50/80 px-4 py-3">
              <p class="min-h-[40px] text-sm leading-relaxed text-gray-600">
                {{ displayedText }}<span class="ml-0.5 inline-block h-4 w-0.5 animate-pulse bg-violet-500" />
              </p>
            </div>
          </div>
        </template>
