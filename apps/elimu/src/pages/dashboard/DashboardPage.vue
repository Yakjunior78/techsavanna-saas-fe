...
const quickActions = [
  { icon: 'book', label: 'Courses', description: 'Manage your courses', color: 'amber' },
  { icon: 'users', label: 'Students', description: 'Student management', color: 'blue' },
  { icon: 'chart', label: 'Reports', description: 'Academic analytics', color: 'emerald' },
  { icon: 'settings', label: 'Settings', description: 'School configuration', color: 'violet' }
]
</script>
    ...
    <main class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div class="mb-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div class="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5">
          ...
          <p class="mt-0.5 text-sm text-amber-100">{{ user?.tenantName || 'My Business' }}</p>
        </div>
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          >
            <!-- Book icon -->
            <svg v-if="action.icon === 'book'" class="size-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"/>
            </svg>
            <!-- Users icon -->
            <svg v-else-if="action.icon === 'users'" class="size-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/>
            </svg>
            <!-- Chart icon -->
            <svg v-else-if="action.icon === 'chart'" class="size-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
