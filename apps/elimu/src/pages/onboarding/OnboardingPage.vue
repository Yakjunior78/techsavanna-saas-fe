...
onMounted(() => {
  fetchPlans('PRODUCT_ELIMU', { description: '', features: [] })
})
      ...
      <div class="space-y-6">
        ...
        <div v-if="provisioningReady" class="py-4 text-center">
          <div class="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-amber-100">
            <svg class="size-8 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
            </svg>
          </div>
          <h2 class="text-xl font-semibold text-gray-900">Your platform is ready!</h2>
          <p class="mt-1.5 text-sm text-gray-500">
            Redirecting in <span class="font-semibold text-amber-600">{{ redirectCountdown }}</span> seconds...
          </p>
          <a
            :href="redirectUrl"
            class="mt-5 inline-flex items-center gap-2 rounded-xl bg-amber-600 px-6 py-3 text-sm font-medium text-white shadow-lg shadow-amber-200 transition-all hover:bg-amber-700 hover:shadow-xl"
          >
